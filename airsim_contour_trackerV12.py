"""
AirSim Drone Racing Lab - Advanced Autonomous Gate Racer v11
===========================================================
Built on v8 (the most stable flight behavior) with three additions:

1. COLLISION RECOVERY (from v10)
   - Detects when the drone is slow (<0.8 m/s) and close to a gate (<4m)
     for more than 1.5 seconds = it hit the gate frame.
   - Backs up 5m along the gate's reverse direction and re-approaches.
   - Up to 2 retries per gate, then skips.

2. YAW-STABILISED GUIDELINE
   - Camera projection uses yaw-only orientation (pitch/roll stripped).
   - The path overlay stays level and aligned with the gates regardless
     of how much the drone tilts during acceleration or banking.

3. All v8 flight parameters and logic preserved exactly.
"""
import math
import re
import time
...
# TUNABLE PARAMETERS
# ---------------------------------------------------------------------------
TAKEOFF_HEIGHT        = -1.5 # More negative makes the drone cruise higher. Less negative makes it cruise lower.

MAX_APPROACH_SPEED    = 12.0 # Higher makes the drone attack the gate setup faster but overshoot more. Lower makes approach safer and easier to control.
MAX_CENTER_SPEED      = 9.0 # Higher makes gate crossing more aggressive but increases the chance of clipping the frame. Lower makes center passes more precise but slower.
MAX_EXIT_SPEED        = 13.0 # Higher carries more momentum to the next gate. Lower makes exit calmer but can make the drone hesitate after a gate.
MIN_SPEED_FRACTION    = 0.20 # Higher keeps more speed even when yaw error is bad, so it pushes through turns harder. Lower lets the drone slow down more for alignment.
MIN_CENTER_SPEED      = 6.0 # Higher prevents the drone from stalling in the gate. Lower allows more careful centering but can cause hovering.
YAW_SCALE_EXPONENT    = 3.3 # Higher penalizes yaw error more strongly, so the drone slows more when not pointed correctly. Lower keeps speed up during turns.
BRAKE_DIST            = 7.5 # Higher starts braking earlier before targets. Lower brakes later and makes lines more aggressive.
BRAKE_MIN_FRACTION    = 0.60 #  Higher keeps more minimum speed while braking. Lower allows stronger slowdowns near the target.
VEL_FF_ALPHA          = 0.02 # Higher makes commands follow current velocity more, which smooths motion but reduces responsiveness. Lower makes path corrections sharper and more immediate.

APPROACH_TOL          = 13.2 # Higher switches out of approach sooner. Lower requires tighter alignment before center phase.
EXIT_TOL              = 18.0 # Higher finishes exit sooner and retargets earlier. Lower stays committed to the exit line longer.
FIRST_APPROACH_DIST   = 5.0 # Higher starts lining up for the first gate farther away. Lower makes the first turn-in later and more aggressive.
APPROACH_DIST         = 3.8 # Higher gives more room to align before gates. Lower creates tighter, faster lines but more misses.
EXIT_DIST             = 0.5 # Higher keeps pushing farther past the gate before retargeting. Lower switches attention to the next gate sooner.
CENTER_THROUGH_DIST   = 4.0 # Higher aims farther through the gate during center phase, which helps prevent hovering. Lower makes the drone treat the gate center more like a stopping point.
GATE_CROSS_EPS        = 0.05 # Higher requires the drone to get farther through the gate plane before switching to exit. Lower switches to exit earlier.
DYN_APPROACH_FRAC     = 0.5 # Higher places approach points farther back relative to gate spacing. Lower places them closer and makes entries tighter.

AUTO_SKIP_PROGRESS    = 2.5 # Higher waits longer before deciding the drone has already gone past a gate. Lower skips sooner if it overshoots.

# Commit zone
COMMIT_DIST           = 2.5 # Higher makes the commit zone activate farther from the gate. Lower activates it only very near the gate.
COMMIT_LAT_DAMP       = 0.80 # Higher keeps more lateral correction during commit. Lower suppresses side corrections more strongly so the drone punches through straighter.
COMMIT_MIN_FWD_SPEED  = 4.5 # Higher forces more forward speed in the commit zone. Lower allows slower gate traversal.
COMMIT_ALT_TOL        = 1.2 # Higher allows commit mode even if altitude is a bit off. Lower requires tighter altitude alignment before commit behavior kicks in.

# Collision recovery (v11 new)
COLLISION_SPEED_THRESH = .8 # Higher makes the script consider the drone “stuck” at a higher speed. Lower only triggers backup when it is almost stopped.
COLLISION_DIST_THRESH  = 4.0 # Higher allows collision recovery to trigger farther from the gate. Lower requires the drone to be very close to the gate first.
COLLISION_TIME_THRESH  = .2 # Higher waits longer before deciding a collision/stuck event happened. Lower triggers backup faster.
BACKUP_DIST           = 5.0 # Higher backs the drone farther away before retrying. Lower does a shorter retreat.
BACKUP_SPEED          = 6.0 # Higher makes backup faster but rougher. Lower makes backup more controlled but slower.
MAX_RETRIES           = 2 # Higher gives more backup-and-retry attempts per gate. Lower skips difficult gates sooner.

# Gate cushion - extra force toward center near gates to prevent edge hits
CUSHION_DIST          = 8.0 # Higher activates edge-avoidance correction farther from the gate. Lower only activates it close to the gate.
CUSHION_THRESHOLD     = 2.5 # Higher ignores more small lateral offsets before applying cushion force. Lower starts correcting sooner.
CUSHION_GAIN          = 16.0 # Higher pushes harder toward the gate center when offset. Lower makes the cushion effect weaker
CUSHION_MAX           = 32.0 # Higher allows stronger maximum cushion correction. Lower caps that force earlier.

# Pure-pursuit lookahead  (v8: all increased for earlier turns)
LOOKAHEAD_MIN         = 1.0 # Higher prevents the lookahead point from being too close, which smooths motion. Lower allows tighter, more reactive tracking.
LOOKAHEAD_MAX         = 15.0 # Higher lets the drone look farther ahead at high speed, which smooths large turns. Lower keeps the target nearer and more reactive.
LOOKAHEAD_SPEED_SCALE = 3.2 # Higher increases lookahead distance as speed rises. Lower keeps the lookahead shorter even at speed.

# Lateral centering (v8: approach gain raised)
LATERAL_GAIN_APPROACH = 1.5 # Higher corrects sideways error more aggressively on approach. Lower makes approach smoother but looser.
LATERAL_GAIN_CENTER   = 7.0 # Higher centers harder while passing the gate. Lower makes center corrections gentler.
LATERAL_MAX           = 6.0 # Higher allows larger sideways correction velocities. Lower limits side motion and reduces aggressive snaps.

# CV-guided lateral correction
CV_STEER_GAIN_APPROACH = 1.2 # Higher gives the camera more influence during approach. Lower relies more on the known gate path.
CV_STEER_GAIN_CENTER   = 3.0 # Higher makes CV correction stronger in the gate itself. Lower keeps the center phase more path-following.
CV_STEER_GAIN_EXIT     = 0.5 # Higher keeps CV influence after passing a gate. Lower reduces CV effect during exit.
CV_STEER_MAX           = 5.5 # Higher allows stronger camera-based steering corrections. Lower caps CV influence sooner.
CV_STEER_BLEND         = 0.5 # Higher trusts CV more compared with geometric lateral correction. Lower trusts the planned path more.

# Yaw alignment
YAW_ALIGN_START_DIST   = 8.0 # Higher starts blending yaw toward gate-forward direction farther away. Lower delays that alignment until closer.
YAW_ALIGN_FULL_DIST    = 2.5 # Higher reaches full gate-forward alignment farther from the gate. Lower waits until very near the gate.

# Altitude  (v8: much more aggressive)
ALT_KP                = 15.0 # Higher makes altitude corrections stronger during approach and exit. Lower makes vertical response slower and smoother.
ALT_KP_CENTER         = 5.5 # Higher makes altitude correction especially aggressive in the gate center phase. Lower makes center-phase altitude behavior gentler.
ALT_MAX_Z_VEL         = 20.0 # Higher allows faster climbs/descents. Lower caps vertical motion and reduces bobbing or dives.

# Stuck detection
STUCK_WINDOW          = 3.0 # Higher watches progress over a longer time before calling the drone stuck. Lower reacts faster.
STUCK_MIN_PROGRESS    = 0.25 # Higher requires more forward progress to avoid a stuck classification. Lower is more forgiving.

# Gate tracker
TRACKER_STALE_S       = 0.5 # Higher keeps old CV detections alive longer when vision disappears. Lower drops stale detections sooner.

# Gate timeout
MAX_GATE_TIME         = 25.0 # Higher gives more time to solve each gate before skipping. Lower skips difficult gates faster.

# Control loop
CONTROL_DT            = 0.02 # Lower updates more often, making control more responsive but potentially twitchier. Higher updates less often, making control calmer but slower.

# CV
CONTOUR_MIN_AREA      = 250 # Higher ignores more small contours and noise. Lower considers more small shapes.
CV_SCORE_THRESHOLD    = 0.30 # Higher requires contours to look more gate-like before using them. Lower accepts weaker detections.
CANNY_LOW             = 50 # Higher removes weak edges and reduces clutter. Lower detects more faint edges and more noise.
CANNY_HIGH            = 130 # Higher keeps only stronger edges. Lower lets more edges through.
...
# Camera projection
CAMERA_FOV_H_DEG      =  90.0 # Higher assumes a wider camera view, which changes how path points project onto the display. Lower assumes a narrower view.

# Path visualisation  (v8: more samples, further ahead)
PATH_VIS_DIST         = 45.0 # Higher draws more of the future path on screen. Lower only shows the near-term path.
PATH_VIS_SAMPLES      = 300 # Higher makes the projected path smoother but costs a bit more compute. Lower makes it coarser.
