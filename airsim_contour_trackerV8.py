"""
AirSim Drone Racing Lab - Advanced Autonomous Gate Racer v8
===========================================================
v8 CHANGES over v7:

1. EARLIER TURNING
   - LOOKAHEAD_SPEED_SCALE 0.9 -> 1.3, MIN 5->7, MAX 14->18
   - VEL_FF_ALPHA 0.12 -> 0.05 (much less velocity smoothing = sharper turns)
   - LATERAL_GAIN_APPROACH 1.8 -> 2.5
   - YAW_SCALE_EXPONENT 1.6 -> 1.3 (less speed penalty for yaw error)

2. ALTITUDE FOLLOWS RACING LINE
   - In APPROACH/EXIT the altitude target is the racing-line lookahead Z,
     which naturally interpolates between gates.  The drone starts climbing
     or descending a full gate segment early.
   - In CENTER the altitude target remains the current gate's Z for precision.
   - ALT_KP 3.5->5.0, ALT_KP_CENTER 4.5->6.5, ALT_MAX_Z_VEL 5->8

3. REFINED GUIDELINE
   - Thicker polyline with green->yellow gradient from near to far
   - Cyan target diamond at the lookahead point
   - Gate labels ("G1","G2",...) at each projected gate center
   - Dotted segments behind the camera are clipped cleanly
"""
import math
import re
import time
    ...
    try:
        while True:
            ...
            target_px_list = project_points_to_camera(
                [target], drone_pos, drone_quat, CAM_W, DASH_H)
