...
YAW_SCALE_EXPONENT    = 1.3     # v8: lowered for less speed penalty on yaw error
BRAKE_DIST            = 3.5
BRAKE_MIN_FRACTION    = 0.30
VEL_FF_ALPHA          = 0.05    # v8: much less smoothing = sharper turns
...
COLLISION_SPEED_THRESH = 0.8     # m/s - below this = possibly stuck on gate
COLLISION_DIST_THRESH  = 4.0     # m - must be close to gate
COLLISION_TIME_THRESH  = 0.5     # seconds of being slow+close before backup
BACKUP_DIST           = 3.0      # metres to reverse along -gate_forward
BACKUP_SPEED          = 5.0      # m/s for backup movement
...
class YawOnlyQuat:
    """Quaternion with only yaw component - strips pitch/roll for stable
    guideline projection that doesn't tilt when the drone banks."""
    ...
    try:
        while True:
            ...
            gate_center_px_list = project_points_to_camera(
                gate_centers_3d, drone_pos, vis_quat, CAM_W, DASH_H)
