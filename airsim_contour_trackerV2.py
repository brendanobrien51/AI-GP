# This script is a smoothed phase-based gate racer for AirSim Drone Racing Lab.
# It reads gate positions directly from the simulator, builds an approach-center-exit
# path for each gate, and then flies through the course using smoothed target points,
# smoothed yaw commands, and smoothed speed commands.
# Compared with the simpler ADRL path-following script, this version adds timing-based
# phase transitions, yaw deadband logic, angle blending, speed smoothing, and separate
# XY/Z smoothing so the drone flies more fluidly and less abruptly.
# OpenCV contour detection is included for visual debugging, but navigation is still
# driven by known simulator gate positions rather than pure camera-based perception.
