"""
AirSim Drone Racing Lab - Advanced Autonomous Gate Racer v14
===========================================================
v14: FIX FOR FLYING PAST GATE EDGES AT HIGH SPEED

Problem: at 14 m/s the drone has ~3x the momentum of 8 m/s but
the lateral correction forces were the same - not enough to bend
the trajectory through the gate center.

Fix: ALL centering forces now scale with speed.
  - Cushion force *= (1 + speed/8) -- doubles at 8 m/s, triples at 16
  - Lateral gains *= (1 + speed/12) -- 50% stronger at 6 m/s
  - CV steer gains *= (1 + speed/10)
  - Cushion range widened to 9m, gain 10, max force 10 m/s
  - Lateral max raised to 10 m/s
  - Stronger yaw alignment starts further out (12m)
"""
import math
import re
import time
...
CUSHION_DIST          = 9.0      # much wider range
CUSHION_THRESHOLD     = 0.2      # trigger on tiny offsets
CUSHION_GAIN          = 10.0     # very strong base pull
CUSHION_MAX           = 10.0     # allow large corrections
CUSHION_SPEED_SCALE   = 8.0      # force *= (1 + speed/this)
...
LATERAL_GAIN_APPROACH = 4.0      # base gain (scaled by speed at runtime)
LATERAL_GAIN_CENTER   = 6.0
LATERAL_MAX           = 10.0     # allow large lateral velocity
LATERAL_SPEED_SCALE   = 12.0     # gain *= (1 + speed/this)
...
CV_STEER_MAX           = 8.0
CV_STEER_BLEND         = 0.6
CV_STEER_SPEED_SCALE   = 10.0    # gain *= (1 + speed/this)

# Yaw alignment - start further out for high speed approaches
...
def cv_steer_correction(tracker, frame_shape, gate_forward, phase,
    ...
    gain *= (1.0 + cur_speed / CV_STEER_SPEED_SCALE)
    if in_commit_zone:
...
def main():
    ...
    try:
        while True:
            ...
            speed_boost = 1.0 + cur_speed / LATERAL_SPEED_SCALE
            ...
            if dist_gate < CUSHION_DIST and phase in ("APPROACH", "CENTER"):
                ...
                if lat_mag > CUSHION_THRESHOLD:
                    ...
                    speed_mult = 1.0 + cur_speed / CUSHION_SPEED_SCALE
                    force_mag  = ((lat_mag - CUSHION_THRESHOLD) *
                                  CUSHION_GAIN * proximity * speed_mult)
                    force_mag  = min(force_mag, CUSHION_MAX * speed_mult)
