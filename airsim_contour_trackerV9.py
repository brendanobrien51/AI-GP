"""
AirSim Drone Racing Lab - Advanced Autonomous Gate Racer v9
===========================================================
v9 CHANGES over v8:

1. GATE-PROXIMITY LOOKAHEAD SCALING
   - Lookahead distance is multiplied by (dist_to_gate / 10), clamped
     to 0.35-1.0.  At 10+ m the full lookahead applies for smooth arcs;
     at 3-4 m the lookahead shrinks to ~35% so the target stays near the
     gate center instead of looking past it onto the next segment.
   - This is the primary fix for the "outside edge clipping" problem.

2. GATE CENTER ATTRACTOR
   - When within 8 m of a gate (APPROACH or CENTER), the target is
     blended up to 60% toward gate_center + fwd*1.5.  Even if the
     racing line is offset, this pulls the drone through center.

3. HIGHER SPEEDS
   - APPROACH 8->11, CENTER 7.5->9, EXIT 10->13, MIN_CENTER 4->5.5
   - Brake later (BRAKE_DIST 3.5->2.5) and less (MIN_FRAC 0.30->0.40)
   - YAW_SCALE_EXPONENT 1.3->1.15 for less speed penalty during turns

4. INCREASED APPROACH_DIST for tighter racing line on turns
"""
import math
import re
import time
...
APPROACH_DIST         = 4.5    # v9: raised from 2.8 for tighter racing line
EXIT_DIST             = 3.0
...
LOOKAHEAD_MIN         = 7.0
LOOKAHEAD_MAX         = 18.0
LOOKAHEAD_SPEED_SCALE = 1.3

# Gate-proximity lookahead scaling (v9 new)
PROX_SCALE_DIST       = 10.0  # dist at which scaling begins
PROX_SCALE_MIN        = 0.35  # minimum scaling factor
PROX_LOOKAHEAD_FLOOR  = 3.0   # absolute minimum lookahead (metres)

# Gate center attractor (v9 new)
GATE_ATTRACT_DIST     = 8.0   # start blending toward gate center
GATE_ATTRACT_MAX      = 0.60  # max blend fraction
GATE_ATTRACT_FWD      = 1.5   # target is center + fwd * this

# Lateral centering
LATERAL_GAIN_APPROACH = 2.5
LATERAL_GAIN_CENTER   = 4.0
LATERAL_MAX           = 6.0
...
TRACKER_STALE_S       = 0.8
MAX_GATE_TIME         = 15.0   # v9: tighter timeout at higher speeds
CONTROL_DT            = 0.08
...
def build_path(client, gate_names):
    ...
    for i, (name, center) in enumerate(gates):
        ...
        if i == 0:
            ...
            base_app = max(2.0, min(APPROACH_DIST, gap * DYN_APPROACH_FRAC))
        path.append({
            "index":    i,
            "name":     name,
            "center":   center.copy(),
...
def score_contour(contour, frame_shape):
    ...
    score = rectangularity * aspect_score * (0.5 + 0.5 * solidity)
...
# ---------------------------------------------------------------------------
# DASHBOARD RENDERING  (compact -- only changed signatures from v8)
# ---------------------------------------------------------------------------
class TopDownMap:
...
def _bar(p, x, y, w, h, frac, fg, label="", val=""):
    frac = clamp(frac, 0.0, 1.0)
    cv2.rectangle(p, (x, y), (x + w, y + h), C_DGRAY, -1)
    cv2.rectangle(p, (x, y), (x + max(2, int(w * frac)), y + h), fg, -1)
    cv2.rectangle(p, (x, y), (x + w, y + h), C_GRAY, 1)
    if label:
        cv2.putText(p, label, (x - 2, y + h - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.34, C_GRAY, 1)
    if val:
        cv2.putText(p, val, (x + w + 4, y + h - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.34, C_WHITE, 1)

def _cbar(p, x, y, w, h, frac, fg, label="", val=""):
    frac = clamp(frac, -1.0, 1.0)
    mid = x + w // 2
    cv2.rectangle(p, (x, y), (x + w, y + h), C_DGRAY, -1)
    fw = int(w / 2 * abs(frac))
    if frac >= 0:
        cv2.rectangle(p, (mid, y), (mid + fw, y + h), fg, -1)
    else:
        cv2.rectangle(p, (mid - fw, y), (mid, y + h), fg, -1)
    cv2.line(p, (mid, y), (mid, y + h), C_GRAY, 1)
    cv2.rectangle(p, (x, y), (x + w, y + h), C_GRAY, 1)
    if label:
        cv2.putText(p, label, (x - 2, y + h - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.34, C_GRAY, 1)
    if val:
...
def draw_brain_panel(phase, gate_idx, total, gate_name,
    ...
    cv2.putText(p, f"Gate {gate_idx+1}/{total}  {gate_name}", (px, y), cv2.FONT_HERSHEY_SIMPLEX, 0.44, C_WHITE, 1)
    y += 12; cv2.line(p, (px, y), (RIGHT_W - px, y), C_DGRAY, 1); y += 8
    _bar(p, bx, y, bw, 13, speed / max(max_speed, 0.1), C_GREEN, "SPEED", f"{speed:.1f}/{max_speed:.0f}"); y += row
    _cbar(p, bx, y, bw, 13, yaw_err_deg / 90.0, C_ORANGE, "YAW ERR", f"{yaw_err_deg:+.1f}d"); y += row
    lf = min(lat_err_mag / 2.5, 1.0)
    _bar(p, bx, y, bw, 13, lf, C_RED if lf > 0.6 else C_YELLOW, "LAT ERR", f"{lat_err_mag:.2f}m"); y += row
    _cbar(p, bx, y, bw, 13, clamp(gate_progress / 4.0, -1, 1), C_GREEN if gate_progress > 0 else C_YELLOW, "PROGRESS", f"{gate_progress:+.2f}m"); y += row
    _cbar(p, bx, y, bw, 13, clamp(z_err_m / 3.0, -1, 1), C_BLUE, "ALT ERR", f"{z_err_m:+.2f}m"); y += row
    _bar(p, bx, y, bw, 13, clamp(1.0 - dist_gate / 25.0, 0, 1), C_BLUE, "DIST", f"{dist_gate:.1f}m"); y += row
    _bar(p, bx, y, bw, 13, clamp(lookahead_d / LOOKAHEAD_MAX, 0, 1), C_LIME, "LOOK-D", f"{lookahead_d:.1f}m"); y += row
    _cbar(p, bx, y, bw, 13, clamp(cv_nx, -1, 1), C_CYAN, "CV STEER", f"x:{cv_nx:+.2f} y:{cv_ny:+.2f}"); y += row
    if detection is not None:
        d = detection
        cc = C_GREEN if d["score"] > 0.65 else (C_YELLOW if d["score"] > 0.45 else C_RED)
        _bar(p, bx, y, bw, 13, d["score"], cc, "CV TOTAL", f"{d['score']:.2f}"); y += 20
    else:
        cv2.putText(p, "CV: NO DETECTION", (bx, y + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.40, C_RED, 1); y += 20
    cv2.line(p, (px, y), (RIGHT_W - px, y), C_DGRAY, 1); y += 8
    sc = C_RED if stuck_det.is_stuck() else C_DGRAY
    cv2.rectangle(p, (px, y), (px + 80, y + 14), sc, -1)
    cv2.putText(p, "STUCK" if stuck_det.is_stuck() else "FLOWING", (px + 4, y + 11), cv2.FONT_HERSHEY_SIMPLEX, 0.38, C_WHITE, 1)
    s3 = norm(vel)
    cv2.putText(p, f"Vx:{vel[0]:+5.1f} Vy:{vel[1]:+5.1f} Vz:{vel[2]:+5.1f}  |V|={s3:.1f}", (px, y + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.36, C_CYAN, 1)
    cv2.putText(p, f"gate_t={elapsed_s:.1f}s", (px, y + 48), cv2.FONT_HERSHEY_SIMPLEX, 0.36, C_GRAY, 1)
    return p
...
def draw_camera_panel(frame, tracker, phase, gate_name, drone_vel,
    ...
    for i in range(n_pts - 1):
        ...
        if p1 is None or p2 is None:
            continue
        if not (-mg < p1[0] < w + mg and -mg < p1[1] < h + mg):
            continue
        if not (-mg < p2[0] < w + mg and -mg < p2[1] < h + mg):
            continue
        t = i / max(n_pts - 1, 1)
        col = (0, 220, int(80 + 140 * t))
        thick = max(1, 3 - int(t * 2))
        cv2.line(cam, p1, p2, col, thick, cv2.LINE_AA)
    for i, pt in enumerate(path_px):
        if pt is not None and 0 <= pt[0] < w and 0 <= pt[1] < h and i % 5 == 0:
            t = i / max(n_pts - 1, 1)
            cv2.circle(cam, pt, 2, (0, 220, int(80 + 140 * t)), -1, cv2.LINE_AA)
    if target_px is not None and 0 <= target_px[0] < w and 0 <= target_px[1] < h:
        cv2.drawMarker(cam, target_px, C_CYAN, cv2.MARKER_DIAMOND, 16, 2)
    for i, gpt in enumerate(gate_center_px_list):
        if gpt is not None and -20 < gpt[0] < w + 20 and -20 < gpt[1] < h + 20:
            ...
            if i < len(gate_labels):
                cv2.putText(cam, gate_labels[i], (gpt[0] + 14, gpt[1] - 6),
    ...
    if det is not None:
        ...
        cv2.circle(cam, (dpx, dpy), 7, bc, -1); cv2.circle(cam, (dpx, dpy), 7, C_WHITE, 1)
        cv2.line(cam, (cw, ch), (dpx, dpy), (0, 200, 120), 1, cv2.LINE_AA)
        stale = tracker.frames_lost > 0
        cv2.putText(cam, f"CV {det['score']:.2f}" + (" [CACHED]" if stale else ""),
                    (dpx + 10, dpy - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.46, bc, 1)
    else:
        cv2.putText(cam, "NO GATE DETECTED", (cw - 110, ch - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, C_RED, 2)
    ...
    spd = norm(drone_vel)
    if spd > 0.3:
        sc = 5.5; ax = int(cw + drone_vel[1] * sc); ay = int(ch - drone_vel[2] * sc)
        cv2.arrowedLine(cam, (cw, ch), (ax, ay), C_CYAN, 2, tipLength=0.28, line_type=cv2.LINE_AA)
        cv2.putText(cam, f"{spd:.1f}m/s", (ax + 6, ay - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.42, C_CYAN, 1)
    yr = math.radians(yaw_err_deg)
    yt = (int(cw + math.sin(yr) * 55), int(ch - math.cos(yr) * 55))
    yc = C_GREEN if abs(yaw_err_deg) < 10 else (C_YELLOW if abs(yaw_err_deg) < 30 else C_RED)
    cv2.line(cam, (cw, ch), yt, yc, 2, cv2.LINE_AA); cv2.circle(cam, yt, 3, yc, -1)
    ...
    cv2.putText(cam, f"-> {gate_name}", (w - 210, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.50, C_WHITE, 1)
    if tracker.frames_lost > 0:
        cv2.putText(cam, f"LOST {tracker.frames_lost}f", (w - 150, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.44, C_RED, 1)
    return cam
...
def build_dashboard(frame, tracker, path, gate_idx, phase, gate,
    ...
    dash[:, :CAM_W] = draw_camera_panel(frame, tracker, phase, gate["name"],
        drone_vel, yaw_err_deg, lat_corr, in_commit, path_px,
        gate_center_px_list, gate_labels, target_px)
    cv2.line(dash, (CAM_W, 0), (CAM_W, DASH_H), C_DGRAY, 1)
    dash[:MAP_H, CAM_W:] = draw_top_down_map(path, gate_idx, drone_pos,
        drone_yaw, target, racing_line, rl_seg_idx)
    cv2.line(dash, (CAM_W, MAP_H), (DASH_W, MAP_H), C_DGRAY, 1)
    dash[MAP_H:, CAM_W:] = draw_brain_panel(phase, gate_idx, len(path),
        gate["name"], speed, max_speed, yaw_err_deg, lat_err_mag,
        gate_progress, dist_gate, tracker.current, drone_vel, gate_elapsed,
        stuck_det, z_err_m, cv_nx, cv_ny, in_commit, lookahead_d)
    return dash

def shutdown(client):
    print("\nShutting down...")
    for fn in [lambda: client.hoverAsync().join(), lambda: client.landAsync().join(),
               lambda: client.disarm(), lambda: client.disableApiControl(),
...
def main():
    ...
    if not gate_names:
        shutdown(client); raise RuntimeError("No gate objects found in scene.")
    ...
    if path:
        g1_app = path[0]["approach"]; g1_z = path[0]["center"][2]
        cur = state_pos(client.getMultirotorState())
        print(f"Pre-race: climbing to z={g1_z:.1f} ...")
        client.moveToPositionAsync(float(cur[0]), float(cur[1]), float(g1_z), 3.0).join()
        print("Pre-race: flying to gate 1 approach ...")
        client.moveToPositionAsync(float(g1_app[0]), float(g1_app[1]), float(g1_z), 3.0).join()
        client.hoverAsync().join(); time.sleep(0.5)
        print("Pre-race alignment complete.")

    cv2.namedWindow("Drone Racing Dashboard", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Drone Racing Dashboard", DASH_W, DASH_H)

    gate_idx = 0; phase = "APPROACH"; gate_start = time.time()
    tracker = GateTracker(); stuck_det = StuckDetector()
    cv_nx_out = 0.0; cv_ny_out = 0.0; rl_seg_idx = 0
    ...
    try:
        while True:
            ...
            drone_vel  = state_vel(state)
            gate_elapsed = time.time() - gate_start
            cur_speed  = norm(drone_vel)
            ...
            hsv_m = multi_hsv_mask(frame); edge_m = edge_mask(frame)
            ...
            if gate_idx >= len(path):
                ...
                cv2.putText(done, "COURSE COMPLETE", (DASH_W // 2 - 230, DASH_H // 2),
                            cv2.FONT_HERSHEY_SIMPLEX, 2.2, C_GREEN, 4)
                cv2.imshow("Drone Racing Dashboard", done); cv2.waitKey(2500)
                break

            gate = path[gate_idx]
            if gate_elapsed > MAX_GATE_TIME:
                print(f"  [TIMEOUT] Skipping gate {gate_idx + 1}")
                gate_idx += 1; phase = "APPROACH"; gate_start = time.time()
                tracker.reset(); stuck_det.reset(); continue
            ...
            if phase in ("APPROACH", "CENTER") and gate_progress > AUTO_SKIP_PROGRESS:
            ...
            # === PURE PURSUIT with gate-proximity scaling ===
            min_seg = gate_idx * 3
            ...
            # v9: SHRINK lookahead near gate to prevent cutting corners
            ...
            # v9: GATE CENTER ATTRACTOR - blend target toward gate
            if phase in ("APPROACH", "CENTER") and dist_gate < GATE_ATTRACT_DIST:
                gate_through = gate["center"] + gate["forward"] * GATE_ATTRACT_FWD
                blend = clamp(1.0 - dist_gate / GATE_ATTRACT_DIST,
                              0.0, GATE_ATTRACT_MAX)
                target = lerp(target, gate_through, blend)
            ...
            if phase == "CENTER":
                ...
                if stuck_det.is_stuck():
                    ...
                    phase = "EXIT"; stuck_det.reset(); continue
            else:
                stuck_det.reset()

            if phase == "APPROACH" and dist_gate < APPROACH_TOL:
                phase = "CENTER"; stuck_det.reset(); continue
            if phase == "CENTER" and gate_progress > GATE_CROSS_EPS:
                phase = "EXIT"; stuck_det.reset(); continue
            if phase == "EXIT":
                past_exit = gate_progress > EXIT_DIST * 0.6
                close_next = (gate_idx + 1 < len(path) and
                    norm(path[gate_idx + 1]["approach"] - drone_pos) < APPROACH_TOL * 1.5)
                if dist_target < EXIT_TOL or past_exit or close_next:
                    gate_idx += 1; phase = "APPROACH"; gate_start = time.time()
                    tracker.reset(); stuck_det.reset(); continue
            ...
            if dist_gate < YAW_ALIGN_START_DIST:
                yb = clamp(1.0 - (dist_gate - YAW_ALIGN_FULL_DIST) /
                           (YAW_ALIGN_START_DIST - YAW_ALIGN_FULL_DIST), 0.0, 0.7)
            ...
            lat_corr = lateral_correction_vec(drone_pos, gate["center"],
                                              gate["forward"], lat_gain, LATERAL_MAX)
            ...
            combined_lat = lerp(lat_corr, cv_corr, CV_STEER_BLEND) if tracker.current else lat_corr

            if in_commit:
                combined_lat = combined_lat * COMMIT_LAT_DAMP
                fc = float(np.dot(vel_desired, gate["forward"]))
                if fc < COMMIT_MIN_FWD_SPEED:
                    vel_desired = vel_desired + gate["forward"] * (COMMIT_MIN_FWD_SPEED - fc)

            vel_cmd = vel_desired + combined_lat
            vel_cmd = lerp(vel_cmd, drone_vel, VEL_FF_ALPHA)
            alt_z = gate["center"][2] if phase == "CENTER" else target[2]
            vel_cmd[2] = altitude_cmd(drone_pos[2], alt_z, phase)

            client.moveByVelocityAsync(
                float(vel_cmd[0]), float(vel_cmd[1]), float(vel_cmd[2]),
                CONTROL_DT * 1.5,
                yaw_mode=airsim.YawMode(is_rate=False,
                                        yaw_or_rate=float(math.degrees(desired_yaw))))

            # Visualisation
            vis_pts = sample_path_ahead(racing_line, rl_seg_idx, rl_seg_t,
                                        PATH_VIS_DIST, PATH_VIS_SAMPLES)
            path_px = project_points_to_camera(vis_pts, drone_pos, drone_quat, CAM_W, DASH_H)
            target_px = (project_points_to_camera([target], drone_pos, drone_quat, CAM_W, DASH_H) or [None])[0]
            gc3d = [path[gi]["center"] for gi in range(gate_idx, min(gate_idx + 4, len(path)))]
            glabels = [f"G{gi+1}" for gi in range(gate_idx, min(gate_idx + 4, len(path)))]
            gc_px = project_points_to_camera(gc3d, drone_pos, drone_quat, CAM_W, DASH_H)

            dash = build_dashboard(frame, tracker, path, gate_idx, phase, gate,
                drone_pos, drone_yaw, drone_vel, target, speed, max_speed,
                speed_scale, yaw_err_deg, lat_err_mag, combined_lat,
                gate_progress, dist_gate, z_err_m, gate_elapsed, stuck_det,
                cv_nx_out, cv_ny_out, in_commit, lookahead_d, racing_line,
                rl_seg_idx, path_px, gc_px, glabels, target_px)
            cv2.imshow("Drone Racing Dashboard", dash)

            cv_label = "%.2f" % tracker.current["score"] if tracker.current else " -- "
            ct = " COMMIT" if in_commit else ""
            print(
                f"G{gate_idx+1:>2}/{len(path)} | {phase:<8}{ct:<7} | "
                f"{gate['name']:<12} | "
                f"dst={dist_target:5.2f} prg={gate_progress:+5.2f} "
                f"z={z_err_m:+5.2f} | "
                f"lk={lookahead_d:4.1f} | "
                f"spd={speed:5.1f}({speed_scale*100:.0f}%) | "
                f"yaw={yaw_err_deg:+6.1f} | cv={cv_label}")
